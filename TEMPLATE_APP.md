...
> 标准模版 = `products/calorieai`（已通过真实 AI / Stripe / SPU 自包含构建全链路验证，无网关运行时依赖）。

## 1. 模版结构（克隆后只需改 3 类文件）

| 改动点 | 文件 | 说明 |
|--------|------|------|
| **App-ID / 品牌 / Prompt / 配色** | `src/lib/app-config.ts` | 应用 ID、品牌名、识图/文字分析 Prompt 工厂、主题主色 |
...
| 5-6 | 商业中台快照 | 无需网关配置：`@git008/commercial-engine` 内联快照随模版复制（升级时同步 `src/lib/commercial-engine/`） |
...
- **商业化中台**：`src/lib/commercial-engine/`（`@git008/commercial-engine` 内联快照：积分 / 收银台 / 限流 / 付费墙）；
