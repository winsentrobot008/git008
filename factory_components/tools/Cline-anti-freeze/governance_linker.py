...
# 👑 008宪法中心 · 白名单准入标记
ACTIVE_PROJECT_MARKER = ".active-project"

# 业务项目目录（开发工位合法操作范围）
# 基于白名单准入标记动态发现，而非硬编码
BUSINESS_DIRS = []  # 将在 boot_check 时通过 _discover_active_projects() 填充


def _discover_active_projects() -> list:
    """
    基于白名单准入标记动态发现活跃项目目录。
    仅返回携带 .active-project 标记的目录（治理中心自身豁免）。
    """
    active = []
    ...
    try:
        # 治理中心自身始终视为活跃
        if ANTI_FREEZE_DIR.exists():
            active.append(ANTI_FREEZE_DIR)
        
        for item in sorted(ROOT_DIR.iterdir()):
            if not item.is_dir() or item.name == "Cline-anti-freeze":
                continue
            marker = item / ACTIVE_PROJECT_MARKER
            if marker.exists():
                active.append(item)
    except OSError:
        pass
...
# ============================================================
# 终端洗消 — Article 1.5 §3 Host Sanitization on Boot
# ============================================================

def zombie_prune() -> Dict:
    """
    全盘终端洗消逻辑 — Article 1.5 §3 Host Sanitization on Boot。

    职责：
    1. 自动检测并物理释放所有未列入活动注册表 (.instance_registry.json) 的
       Cline/ZOOCODE 残留进程。
    2. 验证注册表中所有实例的 PID 是否真实存活，移除已失效的僵尸注册项。
    3. 保留上下文崩溃恢复快照。

    Returns:
        洗消报告字典，包含 killed_pids, cleaned_registry_entries, errors
    """
            ...
            for instance_id, info in registry.items():
                ...
                if pid is not None:
                    if not _is_pid_alive(pid):
                        entries_to_remove.append(instance_id)
                        report["cleaned_registry_entries"].append({
                            "instance_id": instance_id,
                            "pid": pid,
                            "reason": "PID no longer alive",
                        })

            for instance_id in entries_to_remove:
                del registry[instance_id]

            if entries_to_remove:
                INSTANCE_REGISTRY.write_text(
                    json.dumps(registry, ensure_ascii=False, indent=2),
                    encoding="utf-8",
                )

        except Exception as e:
            report["errors"].append(f"Failed to clean registry: {e}")

    # --- Step 2: 检测并释放未注册的 Cline/Python/ZOOCODE 残留进程 ---
    try:
        if sys.platform == "win32":
            # Windows: 检测 python.exe / node.exe 进程树中属于 Cline/ZOOCODE 的部分
            result = sp.run(
                ["tasklist", "/FO", "CSV", "/NH"],
            ...
            if result.returncode == 0:
                for line in result.stdout.strip().split("\n"):
                    ...
                    # 只检测与 Cline/ZOOCODE 相关的进程
                    if not _is_zoo_related_process(pname):
                        continue
                    ...
                    # 检查是否在注册表中
                    if not _is_pid_in_registry(pid):
                        # 未注册的残留进程 — 强制释放
                        kill_ok = _force_kill_pid(pid)
                        if kill_ok:
                            report["killed_pids"].append({
                                "pid": pid,
                                "process": pname,
                                "reason": "Unregistered Cline/ZOOCODE residual process",
                            })
        else:
            # Unix: 扫描 python3/node 进程
            result = sp.run(
                ["ps", "-eo", "pid,comm"],
                capture_output=True, text=True, timeout=15,
            )
        ...
        report["errors"].append(f"Zombie scan failed: {e}")

    report["zombie_pruned"] = True
    return report
...
def _is_pid_alive(pid: int) -> bool:
    ...
    try:
        if sys.platform == "win32":
            ...
            os.kill(pid, 0)  # Unix: signal 0 只检测存在性
            return True
    except (OSError, Exception):
        return False


def _is_zoo_related_process(pname: str) -> bool:
    """判断进程名是否与 Cline/ZOOCODE 相关"""
    zoo_keywords = [
        "python", "node", "powershell", "pwsh",
        "cmd", "code", "cline", "zoocode",
    ]
    pname_lower = pname.lower().replace(".exe", "")
    return any(kw in pname_lower for kw in zoo_keywords)


def _is_pid_in_registry(pid: int) -> bool:
    """检查 PID 是否在活跃注册表中"""
    try:
        if not INSTANCE_REGISTRY.exists():
            return False
...
def boot_check() -> Dict:
    ...
    # 2.1. 基于白名单动态发现活跃项目并更新 BUSINESS_DIRS
    global BUSINESS_DIRS
    BUSINESS_DIRS = _discover_active_projects()
    report["active_projects_discovered"] = [str(d) for d in BUSINESS_DIRS]

    # 2.25. 终端洗消 — Article 1.5 §3 Host Sanitization on Boot
    try:
        zombie_report = zombie_prune()
        report["zombie_prune"] = {
            "executed": True,
            "killed_pids": zombie_report.get("killed_pids", []),
            "cleaned_registry_entries": zombie_report.get("cleaned_registry_entries", []),
            "errors": zombie_report.get("errors", []),
        }
        if zombie_report.get("killed_pids"):
            report["warnings"].append(
                f"终端洗消: 已释放 {len(zombie_report['killed_pids'])} 个未注册残留进程"
            )
        if zombie_report.get("cleaned_registry_entries"):
            report["warnings"].append(
                f"终端洗消: 已清理 {len(zombie_report['cleaned_registry_entries'])} 个僵尸注册项"
            )
    except Exception as e:
        report["zombie_prune"] = {"executed": False, "error": str(e)}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--zombie-prune", action="store_true", help="执行全盘终端洗消（Article 1.5 §3）")
