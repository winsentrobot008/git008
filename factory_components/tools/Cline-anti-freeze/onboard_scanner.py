...
# 👑 008宪法中心 · 白名单准入断言
# ============================================================
# 只有携带 .active-project 标记的目录才被视为"活跃项目"，
# 其余目录一律视为静态备份，哨兵直接路过。
# ============================================================
...
def scan_root_folders() -> List[Path]:
    """
    白名单准入扫描 — 仅识别携带 .active-project 标记的活跃项目目录。
    
    无标记的目录一律视为静态备份或历史存档，哨兵直接路过。
    Cline-anti-freeze/ 自身作为治理中心，豁免此规则。
    """
    folders = []
    ...
    for item in ROOT_DIR.iterdir():
        ...
        # 治理中心自身豁免 — 始终纳入扫描
        if item.name == "Cline-anti-freeze":
            folders.append(item)
            continue
        
        # [白名单准入断言] 无标记则路过
        marker_path = item / ACTIVE_PROJECT_MARKER
        if not marker_path.exists():
            print(f"[whitelist] PASS: {item.name} has no .active-project marker, treating as backup.")
            continue
        
        print(f"[whitelist] ADMIT: {item.name} has .active-project marker, enrolling in monitoring.")
        folders.append(item)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="自主扫描与登记巡检脚本 — Auto-Scan & Register Protocol v2.0 (白名单准入)"
    )
