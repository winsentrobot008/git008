...
def discover_subprojects() -> list[dict]:
    """
    白名单准入扫描 — 仅监控携带 .active-project 标记的活跃项目。
    
    双重断言：
    1. 目录必须存在 .active-project 标记文件
    2. 目录必须存在 .governance_entry.py 哨兵入口
    
    治理中心自身 (Cline-anti-freeze) 豁免 .active-project 要求，
    但必须存在 .governance_entry.py。
    """
    discovered = []
    ...
    try:
        for entry in sorted(ROOT_DIR.iterdir()):
            ...
            # 治理中心自身豁免 .active-project 要求
            if entry.name == "Cline-anti-freeze":
            ...
            # 👑 白名单准入断言
            marker_path = entry / ACTIVE_PROJECT_MARKER
            ...
            # 第二断言：必须存在哨兵入口
            gov_entry = entry / ".governance_entry.py"
            if not gov_entry.exists():
                print(f"[heartbeat_monitor] SKIP: {entry.name} has .active-project but no .governance_entry.py")
                continue
            
            print(f"[heartbeat_monitor] ADMIT: {entry.name} admitted via .active-project, adding to heartbeat monitoring.")
            discovered.append({
                "name": entry.name,
                "path": str(entry),
                "heartbeat_file": str(entry / ".heartbeat"),
