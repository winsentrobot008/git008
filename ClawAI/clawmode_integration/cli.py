"""
CLI entry point for ClawMode — nanobot gateway with ClawAI economic tracking.
...
app = typer.Typer(name="clawmode", help="ClawMode — nanobot + ClawAI economic tracking")


@app.callback()
def _callback() -> None:
    """ClawMode — nanobot gateway with ClawAI economic tracking."""
...
def _build_state(nano_cfg):
    """Construct ClawAIState from the ``agents.clawwork`` plugin config.
    ...
    from clawmode_integration.tools import ClawAIState
...
def _make_agent_loop(nano_cfg, cron_service=None):
    """Create a ClawAIAgentLoop from nanobot config.
...
# -----------------------------------------------------------------------
# Agent command (local CLI — like `nanobot agent` but with ClawAI)
# -----------------------------------------------------------------------
...
def agent(
    ...
    """Chat with the agent locally, with ClawAI economic tracking.
    ...
    console.print(
        f"[bold]ClawAI[/bold] | {state.signature} | "
...
def gateway(
    ...
    """Start the nanobot gateway with ClawAI economic tracking.

    All configuration is read from ~/.nanobot/config.json.
    Enable ClawAI by setting agents.clawwork.enabled = true and
    adjusting token pricing to match your model.

    This launches nanobot's full agent loop with all configured channels
    (Telegram, Discord, Slack, etc.) plus 4 ClawAI economic tools.
