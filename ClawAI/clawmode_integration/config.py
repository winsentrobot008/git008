"""
ClawAI configuration — reads ``agents.clawwork`` from ~/.nanobot/config.json.
...
@dataclass
class ClawAIConfig:
    """ClawAI economic tracking configuration.
    ...
    token_pricing: ClawAITokenPricing = field(default_factory=ClawAITokenPricing)
...
def load_clawwork_config(config_path: Path | None = None) -> ClawAIConfig:
    """Load the ``agents.clawwork`` section from nanobot's config JSON.

    Returns a ``ClawAIConfig`` with defaults for any missing fields.
    """
    path = config_path or _NANOBOT_CONFIG_PATH
    if not path.exists():
        logger.warning(f"Nanobot config not found at {path}, using ClawAI defaults")
        return ClawAIConfig()

    try:
        raw = json.loads(path.read_text())
    except (json.JSONDecodeError, OSError) as exc:
        logger.warning(f"Failed to read {path}: {exc}, using ClawAI defaults")
        return ClawAIConfig()
