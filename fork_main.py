...
async def run_agent(agent: LiveAgent, init_date: str, end_date: str, exhaust: bool = False):
    ...
    try:
        ...
        print(f"鉂?Error running agent {agent.signature}: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
async def main(config_path: str, exhaust: bool = False):
    ...
    print("馃幃 LiveBench - AI Agent Economic Survival Simulation")
    print("=" * 60)
    ...
    if exhaust:
        print(f"馃敟 Mode: EXHAUST 鈥?running all GDPVal tasks (start: {init_date}, ignoring end_date)")
    else:
        print(f"馃搮 Date Range: {init_date} to {end_date}")
    print(f"馃挵 Starting Balance: ${lb_config['economic']['initial_balance']}")

    # Show task pricing configuration
    task_values_path = lb_config.get('economic', {}).get('task_values_path')
    if task_values_path:
        print(f"馃捈 Task Pricing: Real task values from {task_values_path}")
    else:
        default_payment = lb_config.get('economic', {}).get('max_work_payment', 50.0)
        print(f"馃捈 Task Pricing: Uniform ${default_payment} per task (default)")

    print(f"馃捀 Token Pricing: ${lb_config['economic']['token_pricing']['input_per_1m']}/1M input, "
    ...
    if "task_source" in lb_config:
        ...
        print(f"馃搵 Task Source: {task_source['type']}")
        ...
        print("鈿狅笍 DEPRECATION WARNING: 'gdpval_path' is deprecated. Use 'task_source' instead.")
        task_source_config = {
            "task_source_type": "parquet",
            "task_source_path": lb_config["gdpval_path"],
            "inline_tasks": None
        }
        print(f"馃搵 Task Source: parquet (legacy)")
        ...
        print(f"馃搵 Task Source: parquet (default)")
    ...
    if not enabled_agents:
        print("鉂?No agents enabled in configuration")
        return

    print(f"\n馃搵 Enabled Agents: {len(enabled_agents)}")
    ...
    for agent_config in enabled_agents:
        print(f"\n{'='*60}")
        print(f"馃 Initializing Agent: {agent_config['signature']}")
        print(f"{'='*60}\n")
    ...
    print(f"\n{'='*60}")
    print("馃弫 LIVEBENCH SIMULATION COMPLETE")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(args.config):
        print(f"鉂?Configuration file not found: {args.config}")
        sys.exit(1)
    ...
    try:
        ...
        print("\n\n鈿狅笍 Simulation interrupted by user")
        sys.exit(0)
    except Exception as e:
        print(f"\n鉂?Fatal error: {str(e)}")
        import traceback
