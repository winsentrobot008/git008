...
/**
 * SculptProgressCard - the Aura Fit balance dashboard.
 *
 ...
 *
 * Pure presentation: every number comes from lib/aura-fit/balance.ts
 * (computeSculpt), so the dashboard, the per-meal card and the Fit Bestie
 * briefing can never disagree.
 */

import { Sparkles } from "lucide-react";
import { computeSculpt, type SculptProgress } from "@/lib/aura-fit/balance";
...
export default function SculptProgressCard({ consumedKcal, burnedKcal = 0 }: SculptProgressCardProps) {
  ...
  const stateKey =
    sculpt.state === "radiant"
      ? "aura.sculptStateRadiant"
      : sculpt.state === "aligned"
        ? "aura.sculptStateAligned"
        : "aura.sculptStateShaping";
    ...
    <section className="aura-card mt-4 rounded-[28px] p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-base font-black text-ink">{t("aura.sculptSubtitle")}</h3>
        </div>
      ...
      <div className="mt-4 flex items-center gap-5">
        <svg
          ...
          className="aura-silhouette-glow h-40 w-[76px] shrink-0"
          role="img"
          aria-label={t("aura.sculptSilhouetteAlt")}
        >
          <defs>
            <linearGradient id="aura-sculpt-fill" x1="0" y1="1" x2="0" y2="0">
              <stop offset="0%" stopColor="#e6c3cd" />
              <stop offset="60%" stopColor="#d9a7b6" />
              <stop offset="100%" stopColor="#ec4899" />
            </linearGradient>
            <clipPath id="aura-sculpt-clip">
              <path d={SILHOUETTE} />
            </clipPath>
          </defs>
          <path d={SILHOUETTE} fill="rgba(244, 227, 218, 0.8)" />
          <g clipPath="url(#aura-sculpt-clip)">
            <rect x="0" y={fillTop} width={VIEW_WIDTH} height={VIEW_HEIGHT} fill="url(#aura-sculpt-fill)" />
          </g>
          <path
            d={SILHOUETTE}
            fill="none"
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-[11px] font-semibold text-ink-soft">{t("aura.sculptProgressLabel")}</p>
          <p className="mt-3 text-[12px] font-bold leading-snug text-mauve">
            {sculpt.state === "shaping"
              ? t("aura.sculptMinutes", { minutes: sculpt.movementMinutes })
              : t("aura.sculptSettled")}
          </p>
        </div>
      </div>

      <dl className="mt-4 grid grid-cols-3 gap-2">
        <Stat label={t("aura.sculptConsumed")} value={String(sculpt.consumedKcal)} unit="kcal" />
        <Stat
          label={t("aura.sculptBurned")}
          value={sculpt.burnedKcal > 0 ? String(sculpt.burnedKcal) : "-"}
          unit={sculpt.burnedKcal > 0 ? "kcal" : ""}
        />
        <Stat label={t("aura.sculptIdeal")} value={String(sculpt.idealKcal)} unit="kcal" />
      </dl>

      <p className="mt-3 text-[10px] font-semibold leading-relaxed text-ink-faint">
        {t("aura.sculptFootnote")}
      </p>
    </section>
  );
}
