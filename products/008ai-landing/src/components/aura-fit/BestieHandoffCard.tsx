 ...
 * travels with it (see AuraFitApp).
 */

import { ArrowRight, Sparkles } from "lucide-react";
import { BESTIES, otherBestie, type BestieId } from "@/lib/aura-fit/besties";
...
export default function BestieHandoffCard({
  ...
  const copy =
    direction === "intake-to-movement"
      ? {
          title: "aura.handoffToFitTitle",
          body: "aura.handoffToFitBody",
          cta: "aura.handoffToFitCta",
        }
      : {
          title: "aura.handoffToCalorieTitle",
          body: "aura.handoffToCalorieBody",
          cta: "aura.handoffToCalorieCta",
        };
    ...
    >
      <div className="rounded-[26px] bg-white/85 p-5 backdrop-blur">
        <p className="flex items-center gap-1 text-[10px] font-extrabold uppercase tracking-[0.2em] text-mauve">
          ...
          {t("aura.handoffEyebrow", { from: BESTIES[from].name, to: to.name })}
        </p>
