...
import {
  ...
  truncateWords,
  waveformBuckets,
} from "@/lib/calaura/captions";
import type { Bestie } from "@/lib/calaura/besties";
import type { SnippetCopy, VoiceUtterance } from "@/lib/calaura/types";
import {
  downloadClip,
  exportVerticalClip,
...
export default function SnippetStudio({
  ...
  const exportClip = useCallback(async () => {
    ...
    try {
      ...
      downloadClip(clip, `calaurafit-${bestie.id}-${Date.now().toString(36)}`);
      ...
      trackCalauraEvent("calaura_snippet_exported", {
        bestieId: bestie.id,
        width: clip.width,
        height: clip.height,
