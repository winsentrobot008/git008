...
import type { VoiceUtterance } from "@/lib/calaura/types";
...
export function useVoiceEngine(options: VoiceEngineOptions): VoiceEngine {
  ...
  /**
   * Server TTS tier (route: /api/calaura/tts). Used when the platform has no
  ...
  const queueSpeakServer = useCallback(
    async (text: string) => {
      ...
      try {
        const response = await fetch("/api/calaura/tts", {
          method: "POST",
        ...
        console.warn("[calaura] server TTS unavailable:", error);
      } finally {
  ...
  const startTurn = useCallback(async () => {
    ...
    if (recorderSupported) {
      ...
      try {
        ...
        const message = cause instanceof Error ? cause.message : String(cause);
        console.warn("[calaura] recorder unavailable:", message);
      }
    }
    ...
    try {
      ...
      console.warn("[calaura] metering unavailable:", cause);
    }
    ...
    if (Ctor) {
      try {
        ...
        console.warn("[calaura] recognition unavailable:", cause);
      }
    }
