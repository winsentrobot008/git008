...
/**
 * FitBestiePanel - the movement half of the CALauraAI dual-bestie loop.
 *
...
import { BESTIES } from "@/lib/calaura/besties";
...
import { buildIntakeHandoffHref, entryOpeningLine, parseLoopContext, type LoopContext, type LoopLog } from "@/lib/calaura/loop";
import { getSessionId, markLifetimePass } from "@/lib/calaura/quota";
import { MOVEMENT_PRESETS } from "@/lib/calaura/intent";
import { CoachApiError, type DialogueTurn, type VoiceUtterance } from "@/lib/calaura/types";
...
export default function FitBestiePanel({ entry: entryProp = null, onHandoff }: FitBestiePanelProps) {
  ...
  const sendTurn = useCallback(
    async (utterance: VoiceUtterance, emit: (chunk: string) => void): Promise<string> => {
      ...
      try {
        ...
        trackCalauraEvent("calaura_turn_completed", {
          turn: next.used,
          bestieId: bestie.id,
          locked: next.locked,
        });
  ...
  const logMovement = useCallback(() => {
    ...
    trackCalauraEvent("calaura_movement_logged", { label, caloriesBurned: kcal, bestieId: bestie.id });
  ...
  const handoffToCalorie = useCallback(() => {
    ...
    trackCalauraEvent("calaura_movement_handoff", { label, calories: kcal, from: "fit_bestie" });
