/**
 * GET /api/calaura/entitlement?email=...
 *
 * Verifies an 008ai.online Pass against the landing page entitlement store so a
 * paying user can restore access inside CALauraAI. Rate limited to keep the
 * endpoint from being used to enumerate emails.
 */

import { NextRequest, NextResponse } from "next/server";
import { checkRateLimit, clientIp } from "@/lib/calaura/guard";
...
export async function GET(request: NextRequest) {
  ...
  const limit = checkRateLimit(`calaura:entitlement:${ip}`, 12, 60_000);
  if (!limit.allowed) {
