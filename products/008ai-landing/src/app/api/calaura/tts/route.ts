...
import { resolveLanguage } from "@/lib/calaura/config";
import { checkRateLimit, checkUserAgent, clientIp } from "@/lib/calaura/guard";
import { getBestie, type BestieId } from "@/lib/calaura/besties";
...
export async function POST(request: NextRequest) {
  ...
  const limit = checkRateLimit(`calaura:tts:${ip}`, RATE_LIMIT_PER_MINUTE, 60_000);
  if (!limit.allowed) {
  ...
  if (body.mode !== "chunked") {
    try {
      ...
      console.error("[calaura] tts failed:", message);
