/**
 * loop - the contract that wires the CALauraAI dual-bestie cross-loop.
 *
 *   Calorie Bestie  --(generous meal logged)-->  Fit Bestie
 *      /calaura?bestie=fit&food=Pad+Thai&calories=860&from=calorie_bestie
 *
 *   Fit Bestie      --(movement logged)------->  Calorie Bestie
 *      /calaura?bestie=calorie&activity=Pilates&calories=210&from=fit_bestie
 *
 ...
 * render), which keeps /calaura statically prerendered and the SSR markup
...
export const CALAURA_PATH = "/calaura";
...
export function buildMovementHandoffHref({ label, calories }: LoopLog): string {
  ...
  return `${CALAURA_PATH}?${params.toString()}`;
}
