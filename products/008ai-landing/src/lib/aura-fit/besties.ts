/**
 * besties - the two AI besties of Aura Fit (知己轻体).
 *
...
export const BESTIES: Record<BestieId, Bestie> = {
  calorie: {
    ...
    systemPrompt: (ctx) =>
      buildPrompt(
        [
          "You are the CALORIE BESTIE inside Aura Fit: the user's warm, stylish food companion.",
  ...
  fit: {
    ...
    systemPrompt: (ctx) =>
      buildPrompt(
        [
          "You are the FIT BESTIE inside Aura Fit: the user's poised movement companion.",
