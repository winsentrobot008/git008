...
<title>{app_name} - AI-WORKFLOW Live</title>
<style>
...
<div class="app-container">
    <div class="app-header">
        ...
        <p class="app-subtitle">由 AI-WORKFLOW 工厂生成 · 目标: {escaped[:80]}</p>
    ...
    <div class="footer">
        🏭 <span>AI-WORKFLOW Factory</span> · WebSocket Live · <script>document.write(new Date().toLocaleString('zh-CN'));</script>
    </div>
</div>

<script>
// ═══════════════════════════════════════════════════════════
// AI-WORKFLOW WebSocket Live Rendering Engine
// ═══════════════════════════════════════════════════════════
...
function connectWebSocket() {{
    ...
    addLogEntry('SYSTEM', '正在连接 AI-WORKFLOW 工厂...', 'info');

    try {{
        socket = new WebSocket(WS_URL);

        socket.onopen = function() {{
            console.log('[WS] Connected to AI-WORKFLOW Factory');
            setWsStatus('connected');
            addLogEntry('SYSTEM', '✅ 已连接到 AI-WORKFLOW 工厂实时流', 'info');
            if (reconnectTimer) {{
...
def _generate_python_script(description: str) -> str:
    ...
    return (
        f'"""'
        f'{escaped[:80]}...\\n'
        f'Generated by AI-WORKFLOW Factory Worker\\n'
        f'"""\\n'
        f'import json\\n'
        f'import os\\n'
        ...
        f'    print("=" * 60)\\n'
        f'    print("  AI-WORKFLOW Generated Script")\\n'
